...
$env:OTLP_METRICS_ENABLED = 'false'
$agentVersion = '1.56.0'
...
if (-not (Test-Path $agentJar)) {
  ...
  try { Invoke-WebRequest -UseBasicParsing -Uri $url -OutFile $agentJar } catch { & curl.exe -L -o $agentJar $url }
}
...
java -javaagent:$agentJar -jar target/todo-otel-openobserve-0.0.1-SNAPSHOT.jar